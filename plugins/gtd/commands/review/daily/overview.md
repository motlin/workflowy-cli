...
The reconnect instruction must be a plain-text response that ends the turn. Do **not** use `AskUserQuestion`: an open prompt blocks the user from typing `/mcp` until they press Esc. Display the reconnect instruction and return control immediately.
...
- The Reminders/AppleScript bridge **stops responding to writes** after a handful of them — `osascript` hangs until its timeout kills it, exits 124, and applies nothing, while `reminders_fetch` keeps reading fine the whole time. Quitting and relaunching Reminders.app clears it for the next few writes. Batch all reminder writes into one script rather than one per item; see **Apple Reminders writes freeze** in `daily/due.md`. Always confirm through `reminders_fetch`, never through the script's exit code.
- **`reminders_create` drops the due date.** A reminder created through iMCP comes back with no `scheduledTime`, so no alarm ever fires. To create a reminder that actually alerts at a time, use Fantastical instead: `osascript -e 'tell application "Fantastical" to parse sentence "reminder <title> today at 10:07am" with add immediately'`, then verify with `reminders_fetch`. Fantastical also exits 0 on failure, so the verification is not optional — a calendar-event parse silently created nothing in the 2026-08-21 review and had to be redone with `events_create`.
