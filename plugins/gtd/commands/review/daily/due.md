...
For a Workflowy row whose date has passed without external-deadline evidence, keep **Move to asap ladder** immediately after **Done**, including when it has a skip streak. Preserve its high priority with the tier choice above. A streak alone does not establish an external deadline or justify lowering priority. For Things and Reminders work with no deadline, keep **Clear the date** available.
